#这段代码定义了一个名为`MemReporter`的类，其作用是**监控和报告PyTorch程序中的内存使用情况**，特别是针对张量（tensor）的内存分配。
import math
import gc
...
try:
    ...
    def calmsize(num_bytes: int) -> str:
...
def readable_size(num_bytes: int) -> str:
    ...
    return size if isinstance(size, str) else '{:.2f}'.format(size)
