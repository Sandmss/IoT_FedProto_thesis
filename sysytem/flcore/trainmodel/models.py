...
class BaseHeadSplit(nn.Module):
    def __init__(self, args, cid):
        ...
        self.packet_base = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=(3, 3), padding=1, bias=False),
            nn.GroupNorm(2, 16), # 直接使用 GroupNorm 适应联邦学习数据异构
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=(2, 2)), # 输出形状变为 16 x 4 x 20

            nn.Conv2d(16, 32, kernel_size=(3, 3), padding=1, bias=False),
            nn.GroupNorm(4, 32),
            nn.ReLU(inplace=True),

            # 使用自适应全局池化，将其强行压缩为 1x1 空间大小，确保尺寸稳定性
            nn.AdaptiveAvgPool2d((1, 1)), # 输出形状变为 32 x 1 x 1
            nn.Flatten(),

            # 最终映射到与流级特征相同的维度 (args.feature_dim)，方便联合计算和距离度量
            nn.Linear(32, args.feature_dim),
            nn.ReLU(inplace=True)
        )
        # 使用你文件中定义的初始化函数进行初始化
        self.packet_base.apply(init_weights)
...
class CNN(nn.Module):
    # 修改默认 in_features=3, height=40 以适配新数据
    def __init__(self, in_features=3, num_classes=10, height=40, 
...
class FedAvgCNN(nn.Module):
    # 修改默认 in_features=3, dim=3136 以适配新数据 (40x40x3)
    def __init__(self, in_features=3, num_classes=10, dim=3136):
        super().__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_features,
                        32,
...
class FedAvgMLP(nn.Module):
    # 修改 in_features=4800 (3 * 40 * 40)
    def __init__(self, in_features=4800, num_classes=10, hidden_dim=200):
        super().__init__()
...
class Mclr_Logistic(nn.Module):
    # 修改 input_dim=4800 (3 * 40 * 40)
    def __init__(self, input_dim=4800, num_classes=10):
...
class DNN(nn.Module):
    # 修改 input_dim=4800 (3 * 40 * 40)
    def __init__(self, input_dim=4800, mid_dim=100, num_classes=10):
...
class LeNet(nn.Module):
    # 针对 40x40 输入，经过两次 5x5卷积和池化，feature_dim 变为 50*7*7=2450
    def __init__(self, feature_dim=2450, bottleneck_dim=256, num_classes=10, iswn=None):
        super(LeNet, self).__init__()

        self.conv_params = nn.Sequential(
            nn.Conv2d(3, 20, kernel_size=5), # 修改输入为3通道
            nn.MaxPool2d(2),
            nn.ReLU(),
